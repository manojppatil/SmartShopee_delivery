...
public class NewOrderFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        final View view = inflater.inflate(R.layout.fragment_neworder, container, false);
        rotateLoading = (RotateLoading) view.findViewById(R.id.rotateloading);
        AsyncHttpClient asyncHttpClient = new AsyncHttpClient();
        asyncHttpClient.post(Routes.neworder, new AsyncHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
                ...
                try {
                    if (statusCode == 200) {
                        try {
                            ...
                            NeworderAdapter newoderadapter = new NeworderAdapter(arr, context);
                            RecyclerView recyclerView = (RecyclerView) view.findViewById(R.id.neworder_recycler);
                            recyclerView.setHasFixedSize(true);
                            recyclerView.setLayoutManager(new LinearLayoutManager(context));
                            recyclerView.setItemAnimator(new DefaultItemAnimator());
                            recyclerView.setAdapter(newoderadapter);
